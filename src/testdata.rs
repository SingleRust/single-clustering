...
/// Edges of Zachary's karate club, the standard community-detection benchmark.
...
/// The known modularity-optimal 4-community partition of the karate club, Q = 0.4198.
pub fn karate_optimum() -> Vec<usize> {
    vec![
...
/// A stochastic block model: `blocks` groups of `n_per` nodes, with within-group edge
/// probability `p_in` and between-group probability `p_out`. Returns the edges and the
/// planted labels.
pub fn sbm(
    n_per: usize,
    blocks: usize,
...
/// Counts communities that are not internally connected — the guarantee Leiden's refinement
